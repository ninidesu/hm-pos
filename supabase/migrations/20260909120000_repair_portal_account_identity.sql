-- HM POS · align existing cashier checkout with direct portal accounts
--
-- Directly created admin/cashier accounts live in public.users. This repair
-- keeps checkout tied to auth.uid() and uses the canonical users-based staff
-- check. It does not send email, OTP, invitation, or invoke an Edge Function.
...
-- Older deployments have this implementation from the cashier order
-- hardening migration. The canonical installer already uses the users-based
-- check, so only replace the older internal function when it exists.
do $migration$
begin
  if to_regprocedure('public.create_cashier_order_internal(jsonb)') is not null then
    execute $cashier_function$
