...
const fromName = Deno.env.get("MAIL_FROM_NAME") || "HM POS";
const storeName = Deno.env.get("STORE_NAME") || "HM POS";
...
const bannerImage = Deno.env.get("MAIL_BANNER_IMAGE") || "";
...
const getCaller = async (req: Request) => {
  ...
  const users = await rest(`users?id=eq.${encodeURIComponent(user.id)}&select=role&limit=1`, {
    method: "GET",
  });
  return { id: user.id as string, role: String(users?.[0]?.role || "customer"), isService: false };
};
...
const buildEmail = (event: OutboxEvent) => {
  ...
  const content: Record<string, { subject: string; title: string; intro: string; detail: string; status: string; tone: string }> = {
    order_cancelled: {
      subject: `HM POS - Order ${orderNumber} cancelled`,
    ...
    cancellation_requested: {
      subject: `HM POS - Cancellation review for ${orderNumber}`,
    ...
    cancellation_approved_refund_pending: {
      subject: `HM POS - Refund pending for ${orderNumber}`,
    ...
    cancellation_rejected: {
      subject: `HM POS - Cancellation update for ${orderNumber}`,
    ...
    refund_processed: {
      subject: `HM POS - Refund completed for ${orderNumber}`,
