...
begin
  ...
  select s.* into v_stock from public.stock s where s.id = p_stock_id for update;
  if not found then raise exception 'Stock record not found'; end if;
  select name into v_item_name from public.menu_items where id = v_stock.menu_item_id;
  select coalesce(username, full_name, email, 'Unknown user'), role into v_actor_name, v_actor_role from public.users where id = auth.uid();
