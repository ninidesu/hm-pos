...
function printRefundReport({ records, summary, rangeLabel }) {
  ...
  </style></head><body><h1>HM POS - Refunds Report</h1><p>${escape(rangeLabel)} - Generated ${escape(new Date().toLocaleString('en-PH'))}</p>
...
function reportDefinition(view) {
  return view === 'refunds'
    ? { label: 'Refunds', title: 'HM POS - REFUNDS REPORT', sheet: 'Refund Ledger', columns: refundReportColumns }
    : { label: 'Cancellations', title: 'HM POS - CANCELLATIONS REPORT', sheet: 'Cancellation Ledger', columns: cancellationReportColumns }
}

export async function exportCancellationReportToXlsx({ records = [], summary = {}, rangeLabel = 'Selected period', view = 'cancellations', generatedBy = 'HM POS' }) {
  ...
  downloadBlob(new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }), `hm-pos-${slug}-report-${new Date().toISOString().slice(0, 10)}.xlsx`)
}

export async function exportCancellationReportToPdf({ records = [], summary = {}, rangeLabel = 'Selected period', view = 'cancellations', generatedBy = 'HM POS' }) {
  ...
  const slug = view === 'refunds' ? 'refunds' : 'cancellations'
  downloadBlob(pdf.output('blob'), `hm-pos-${slug}-report-${new Date().toISOString().slice(0, 10)}.pdf`)
}
...
export function printCancellationReport({ records, summary, rangeLabel, view = 'cancellations' }) {
  ...
  reportWindow.document.write(`<!doctype html><html><head><title>Cancellation Report</title><style>
    ...
    article{border:1px solid #dfe4dd;border-radius:12px;padding:14px;flex:1}article span{font-size:11px;color:#68736b;text-transform:uppercase}article b{display:block;font-size:20px;margin-top:8px}
    table{width:100%;border-collapse:collapse;font-size:11px}th,td{padding:9px;border-bottom:1px solid #dfe4dd;text-align:left}th{background:#f5f7f3;text-transform:uppercase}
  </style></head><body><h1>HM POS - Cancellation Report</h1><p>${escape(rangeLabel)} · Generated ${escape(new Date().toLocaleString('en-PH'))}</p>
