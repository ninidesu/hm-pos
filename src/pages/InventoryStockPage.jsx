...
export default function InventoryStockPage({ role = 'admin' }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      if (query && ![item.name, item.sku, item.supplier].join(' ').toLowerCase().includes(query)) return false
  ...
  return <AppShell role={role} title="Stock Management" eyebrow="Track sellable item quantities and reorder levels." onRefresh={load}>
    <section className="stock-page">
      ...
      <section className="stock-panel" aria-label="Stock records">
        <div className="stock-toolbar"><label className="stock-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search menu item, SKU, supplier" aria-label="Search stock" /></label><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} aria-label="Filter stock status"><option value="all">All stock states</option><option value="out">Out of stock</option><option value="low">Low stock</option><option value="healthy">Healthy</option></select><button type="button" className="stock-add-button" onClick={openNew}><Plus size={16} /> Add stock</button></div>
        {loading ? <div className="stock-empty">Loading stock…</div> : filtered.length === 0 ? <div className="stock-empty"><Package size={28} /><b>No stock records found</b><span>Add a stock record to connect a menu item to quantity tracking.</span></div> : <div className="stock-table-wrap"><table className="stock-table"><thead><tr><th>Menu item</th><th>On hand</th><th>Reorder at</th><th>Status</th><th>Updated</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{filtered.map((item) => { const status = STATUS[stockState(item)]; return <tr key={item.id}><td><b>{item.name}</b><small>{item.sku || item.unit}</small></td><td><strong>{formatQuantity(item.quantity)}</strong> {item.unit}</td><td>{formatQuantity(item.minStockLevel)} {item.unit}</td><td><span className={`stock-status ${status.className}`}>{status.label}</span></td><td>{item.updatedAt ? new Date(item.updatedAt).toLocaleDateString('en-PH') : '—'}</td><td><div className="stock-actions"><button type="button" className="icon-button" onClick={() => setAdjustment({ id: item.id, name: item.name, current: item.quantity, delta: '', reason: '' })} aria-label={`Adjust ${item.name}`}><Package size={15} /></button><button type="button" className="icon-button" onClick={() => openEdit(item)} aria-label={`Edit ${item.name}`}><Pencil size={15} /></button></div></td></tr> })}</tbody></table></div>}
      </section>
    </section>
    {form && <Modal title={form.id ? 'Edit stock record' : 'Add stock record'} onClose={() => !saving && setForm(null)}><form onSubmit={save} className="stock-form"><label>Menu item<select required value={form.menuItemId} onChange={(event) => setForm((current) => ({ ...current, menuItemId: event.target.value }))} disabled={Boolean(form.id)}><option value="">Select a menu item</option>{menuItems.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><div className="stock-form-grid"><label>Quantity<input type="number" min="0" step="0.01" value={form.quantity} onChange={(event) => setForm((current) => ({ ...current, quantity: event.target.value }))} /></label><label>Reorder level<input type="number" min="0" step="0.01" value={form.minStockLevel} onChange={(event) => setForm((current) => ({ ...current, minStockLevel: event.target.value }))} /></label><label>Healthy level<input type="number" min="0" step="0.01" value={form.highStockLevel} onChange={(event) => setForm((current) => ({ ...current, highStockLevel: event.target.value }))} /></label><label>Unit<input maxLength="20" value={form.unit} onChange={(event) => setForm((current) => ({ ...current, unit: event.target.value }))} /></label></div><div className="stock-form-grid"><label>SKU<input maxLength="60" value={form.sku} onChange={(event) => setForm((current) => ({ ...current, sku: event.target.value }))} /></label><label>Supplier<input maxLength="120" value={form.supplier} onChange={(event) => setForm((current) => ({ ...current, supplier: event.target.value }))} /></label><label>Expiration date<input type="date" value={form.expirationDate} onChange={(event) => setForm((current) => ({ ...current, expirationDate: event.target.value }))} /></label><label>Cost per unit<input type="number" min="0" step="0.01" value={form.costPerUnit} onChange={(event) => setForm((current) => ({ ...current, costPerUnit: event.target.value }))} /></label></div><label>Notes<textarea rows="3" maxLength="500" value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} /></label><ModalActions saving={saving} onClose={() => setForm(null)} /></form></Modal>}
