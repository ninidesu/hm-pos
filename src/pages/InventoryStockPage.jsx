...
export default function InventoryStockPage({ role = 'admin' }) {
  ...
  return <AppShell role={role} title="Stock Management" eyebrow="Track sellable item quantities and low-stock indicators." onRefresh={load}>
    ...
    {form && <Modal title={form.id ? 'Edit stock record' : 'Add stock record'} onClose={() => !saving && setForm(null)}><form onSubmit={save} className="stock-form"><label>Menu item<select required value={form.menuItemId} onChange={(event) => setForm((current) => ({ ...current, menuItemId: event.target.value }))} disabled={Boolean(form.id)}><option value="">Select a menu item</option>{menuItems.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><div className="stock-form-grid"><label>Quantity<input type="number" min="0" step="0.01" value={form.quantity} onChange={(event) => setForm((current) => ({ ...current, quantity: event.target.value }))} /></label><label>Low stock indicator<input type="number" min="0" step="0.01" value={form.minStockLevel} onChange={(event) => setForm((current) => ({ ...current, minStockLevel: event.target.value }))} /></label><label>Healthy level<input type="number" min="0" step="0.01" value={form.highStockLevel} onChange={(event) => setForm((current) => ({ ...current, highStockLevel: event.target.value }))} /></label><label>Unit<input maxLength="20" value={form.unit} onChange={(event) => setForm((current) => ({ ...current, unit: event.target.value }))} /></label></div><label>Notes<textarea rows="3" maxLength="500" value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} /></label><ModalActions saving={saving} onClose={() => setForm(null)} onRemove={form.id ? () => setAdjustment({ id: form.id, name: menuItems.find((item) => item.id === form.menuItemId)?.name || 'this item', current: Number(form.quantity), unit: form.unit || 'piece', amount: '', reason: '' }) : null} /></form></Modal>}
    {adjustment && !removeConfirmOpen && <Modal title={`Remove stock from ${adjustment.name}`} onClose={() => !saving && setAdjustment(null)}><form onSubmit={requestRemoval} className="stock-form"><p className="stock-adjustment-copy">Available stock: <b>{formatQuantity(adjustment.current)} {adjustment.unit}</b>. Enter how much should be deducted.</p><label>Amount to remove<input required type="number" min="0.01" max={adjustment.current} step="0.01" value={adjustment.amount} onChange={(event) => setAdjustment((current) => ({ ...current, amount: event.target.value }))} autoFocus /></label><label>Reason<input required maxLength="160" value={adjustment.reason} onChange={(event) => setAdjustment((current) => ({ ...current, reason: event.target.value }))} placeholder="Waste, damage, count correction…" /></label><footer className="stock-modal-actions"><button type="button" className="secondary-button" onClick={() => setAdjustment(null)}>Cancel</button><button type="submit" className="danger-button"><PackageMinus size={16}/>Continue</button></footer></form></Modal>}
