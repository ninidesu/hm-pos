...
export default function StaffSettingsPage({ role = 'staff' }) {
  ...
  async function submitPassword(event) {
    ...
    if (!isValidInternalPassword(passwords.password)) { showNotice('error', 'Use 8–12 characters.'); return }
  ...
  return <AppShell role={role} title="Settings" onRefresh={refreshSettings}>
    ...
    {loading ? <div className="staff-settings-loading">Loading your settings…</div> : <section className="staff-settings-container" aria-label={`${roleLabel(role)} settings`}>
      ...
      {activeSection === 'security' && <form className="staff-settings-card staff-settings-security" onSubmit={submitPassword}>
        ...
        <div className="staff-settings-grid">
          <label className="staff-settings-field" htmlFor="current-password"><span>Current password</span><input id="current-password" type="password" value={passwords.current} onChange={(event) => setPasswords((current) => ({ ...current, current: event.target.value.slice(0, 12) }))} autoComplete="current-password" maxLength="12" required /></label>
          <label className="staff-settings-field" htmlFor="new-password"><span>New password</span><input id="new-password" type="password" value={passwords.password} onChange={(event) => setPasswords((current) => ({ ...current, password: event.target.value.slice(0, 12) }))} autoComplete="new-password" minLength="8" maxLength="12" pattern=".{8,12}" required /><small>Use 8–12 characters.</small></label>
          <label className="staff-settings-field" htmlFor="confirm-password"><span>Confirm new password</span><input id="confirm-password" type="password" value={passwords.confirm} onChange={(event) => setPasswords((current) => ({ ...current, confirm: event.target.value.slice(0, 12) }))} autoComplete="new-password" minLength="8" maxLength="12" pattern=".{8,12}" required /></label>
        </div>
