  ...
  return <div className="hm-admin-dashboard">
    ...
    <section className="hm-dashboard-primary-grid">
      ...
      <article className="hm-dashboard-panel hm-dashboard-transactions">
        ...
        <div className="hm-dashboard-table-wrap"><table><thead><tr><th>Transaction</th><th>Items</th><th>Payment</th><th>Total</th><th>Status</th><th>Time</th></tr></thead><tbody>{metrics.recentOrders.slice(0, 7).map((order) => { const items = order.order_items || []; return <tr key={order.id}><td><b>{order.order_number}</b></td><td>{items.length ? items.map((item) => item.display_name || item.item_name).slice(0, 2).join(', ') : 'No item details'}</td><td>{dashboardPaymentMethod(order.payments)}</td><td><b>{money(order.final_total)}</b></td><td><span>{order.order_source === 'cashier_pos' ? 'Completed' : order.status}</span></td><td>{formatTime(order.created_at)}</td></tr> })}</tbody></table>{!metrics.recentOrders.length && <SimpleEmpty icon={ReceiptText} text="No recent transactions yet." />}</div>
      </article>
