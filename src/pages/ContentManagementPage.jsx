import { AlertTriangle, Check, CheckCircle2, ClipboardCheck, Eye, FileText, Home, Info, MessageSquareQuote, Save, Sparkles, XCircle } from 'lucide-react'
import { useCallback, useEffect, useMemo, useState } from 'react'
import AppShell from '../components/AppShell'
import { describeError } from '../utils/describeError'
import { fetchMenuApprovalRequests, updateMenuApprovalRequest } from '../services/menuApprovalService'
import {
  CONTENT_DEFAULTS, fetchContentMenuOptions, fetchPortalConfiguration, savePortalConfiguration,
...
export default function ContentManagementPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [configuration, menuItems] = await Promise.all([
        fetchPortalConfiguration('content'), fetchContentMenuOptions(),
      ])
      setContent(configuration.values); setUpdatedAt(configuration.updatedAt); setSetupRequired(configuration.setupRequired)
      setMenu(menuItems); setError('')
...
function matchesApprovalChangeType(item, filter) {
  ...
  const terms = {
    ...
    category: ['categor'], temperature: ['temperature'], display: ['display'],
  }
