import { AlertTriangle, Check, Clock3, CreditCard, Database, Image, Info, RotateCcw, Save, ShieldCheck, ShoppingBag, Store, Upload } from 'lucide-react'
  ...
  const load = useCallback(async () => {
    ...
    try {
      const configuration = await fetchPortalConfiguration('system')
       setSettings((current) => ({ ...current, ...configuration.values, store: { ...current.store, ...configuration.values.store, phone: sanitizePhone(configuration.values.store?.phone || '') } })); setUpdatedAt(configuration.updatedAt); setSetupRequired(configuration.setupRequired); setError('')
          ...
          {loading ? <div className="ac-skeleton"><i/><i/><i/><i/></div> : <>
            ...
            {section === 'ordering' && <SettingsSection title="Hours & ordering" description="Control when customers can place orders and which fulfillment methods appear." onSave={() => save('ordering')} saving={saving}>
              ...
              <div className="ac-form-grid">
                ...
                <Field label="Minimum order (PHP)" hint="Set to 0 for no minimum."><input type="text" inputMode="decimal" pattern="[0-9]*\.?[0-9]*" value={settings.ordering.minimumOrder} onChange={(event) => update('ordering', { minimumOrder: sanitizeDecimal(event.target.value) })}/></Field>
            ...
            {section === 'payments' && <SettingsSection title="Payments" description="Choose the methods customers can use and keep instructions accurate." onSave={() => save('payments')} saving={saving}>
              ...
              <div className="ac-subsection"><h3>Cash on delivery</h3><div className="ac-form-grid"><Field label="Maximum order total (PHP)" hint="Orders above this amount must use a digital method."><input type="text" inputMode="decimal" pattern="[0-9]*\.?[0-9]*" value={settings.payments.codMaximum} onChange={(event) => update('payments', { codMaximum: sanitizeDecimal(event.target.value) })}/></Field></div></div>
            ...
            {section === 'security' && <section className="ac-editor-section"><header><div><h2>Platform security</h2><p>Security controls are shown where they are actually managed.</p></div></header><div className="ac-security-grid">
              ...
              <article><span><ShoppingBag size={20}/></span><div><b>Change history</b><p>Administrative changes remain attributable through the portal activity log.</p><a href="/admin/settings/activity">Open Activity Logs</a></div></article>
            </div></section>}
          </>}
        </main>
      </div>
