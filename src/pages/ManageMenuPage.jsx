...
export default function ManageMenuPage({ role = 'staff' }) {
  const isAdmin = role === 'admin'
  const storagePrefix = isAdmin ? 'admin' : 'staff'
  ...
  const [maxPrice, setMaxPrice] = useManagementSessionState(`${storagePrefix}:menu:max-price`, '')
  const [sortBy, setSortBy] = useManagementSessionState(`${storagePrefix}:menu:sort`, 'name')
  const [view, setView] = useManagementSessionState(`${storagePrefix}:menu:view`, 'grid')
  const [selectedIds, setSelectedIds] = useManagementSessionState(`${storagePrefix}:menu:selected-items`, [])
  const [menuOpenId, setMenuOpenId] = useState('')

  const [formTarget, setFormTarget] = useManagementSessionState(`${storagePrefix}:menu:item-form`, null)
  ...
  const load = async () => {
    ...
    try {
      if (!isSupabaseConfigured) {
        setMainCategories([]); setSubcategories([]); setItems([])
        setError('Supabase is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to load the menu.')
        return
      }
  ...
  const handleDuplicate = (item) => {
    ...
    requestApproval({ action: 'add', itemName: `${item.name} copy`, summary: 'Create a duplicate menu item', changeTypes: ['New item'], operationKey: 'duplicate_menu_item', payload: { id: item.id } }, () => runDuplicate(item))
      .then(() => pushToast('success', `${item.name} duplication sent for admin review.`))
      .catch(() => {})
  }

  const saveMenuItem = async (payload) => {
    const id = await upsertMenuItem(payload)
    await load()
  ...
  const handleArchive = (target) => {
    ...
    requestApproval({ action: 'remove', itemName: target.name, summary: 'Remove this menu item from active listings', changeTypes: ['Item removal'], operationKey: 'archive_menu_item', payload: { id: target.id } }, () => runArchive(target))
      .then(() => pushToast('success', `${target.name} removal sent for admin review.`))
      .catch(() => {})
  }

  return (
    <AppShell role={role} title="Manage Menu" onRefresh={load} actions={
      ...
      <section className="inv-summary-row menu-summary-grid" aria-label="Menu overview">
        <article className="inv-summary-card menu-summary-card accent-purple">
      ...
      {formTarget && (
        <ItemFormModal
          item={formTarget.item} mainCategories={mainCategories} subcategories={subcategories} isAdmin={isAdmin}
          ...
          <section className="payment-modal ops-popup-modal menu-availability-modal" role="dialog" aria-modal="true" aria-labelledby="menu-availability-title" aria-describedby="menu-availability-description">
            ...
            <p id="menu-availability-description">
              ...
              {availabilityTarget.manualAvailable
                ...
                : availabilityTarget.isBulk ? 'All selected items will be available for ordering.' : 'Customers will be able to see and order this item again.'}
            </p>
      ...
      {archiveTarget && (
        <div className="payment-modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget && busyId !== archiveTarget.id) setArchiveTarget(null) }}>
          <section className="payment-modal" role="alertdialog" aria-modal="true" aria-labelledby="menu-archive-title">
            <span className="payment-modal-kicker">{isAdmin ? 'Confirm archive' : 'Admin review required'}</span>
            <h2 id="menu-archive-title">Delete {archiveTarget.name}?</h2>
            <p>{isAdmin ? 'The item will be archived to preserve past orders and receipts.' : 'This deletion must be reviewed by an administrator before it is applied. The item will be archived to preserve past orders and receipts.'}</p>
            <div className="payment-modal-actions">
              ...
              <button className="danger-button" type="button" disabled={busyId === archiveTarget.id} onClick={() => handleArchive(archiveTarget)}>{busyId === archiveTarget.id ? 'Archiving…' : isAdmin ? 'Archive item' : 'Send for review'}</button>
            </div>
          </section>
        </div>
      )}
      {categoryManagerOpen && (
        <CategoryManagerModal
          ...
          onChanged={load} pushToast={pushToast} requestApproval={requestApproval} isAdmin={isAdmin}
        />
      )}
            ...
            <div className="menu-editor-panel">
              {section === 'basics' && (
                <section id="menu-editor-basics" role="tabpanel" className="menu-form-section" aria-label="Basic item details">
                  <header><div><h3>Basic details</h3><p>The information customers use to identify this item.</p></div></header>
                  ...
                  <div className="form-grid menu-form-grid">
                    ...
                    <label className="field"><span>Menu price incl. VAT (PHP)</span><input type="text" inputMode="decimal" pattern="[0-9]*\.?[0-9]*" min="0" step="0.01" value={values.price} onChange={(e) => set('price', sanitizeDecimal(e.target.value))} placeholder="0.00" required /></label>
                    ...
                    {values.choicesEnabled ? <div className="menu-choice-editor">
                      ...
                      <div className="menu-choice-list">
                        {choices.map((choice, index) => <div className="menu-choice-row" key={choice.key || index}>
                          ...
                          <label className="field"><span>Price incl. VAT (PHP)</span><input type="text" inputMode="decimal" pattern="[0-9]*\.?[0-9]*" min="0" step="0.01" value={choice.price} onChange={(e) => set('choices', choices.map((current, choiceIndex) => choiceIndex === index ? { ...current, price: sanitizeDecimal(e.target.value) } : current))} placeholder="165.00" /></label>
...
function CategoryManagerModal({ mainCategories, subcategories, isAdmin = false, onClose, onChanged, pushToast, requestApproval }) {
