  ...
  return <div className="ops-drawer-backdrop cancel-drawer-backdrop" onMouseDown={onClose}>
    <aside className="ops-drawer txn-drawer cancel-drawer" role="dialog" aria-modal="true" aria-label={`${view === 'refunds' ? 'Refund' : 'Cancellation'} record ${record.orderNumber}`} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="ops-drawer-body">
        ...
        <section><h3>Amount breakdown</h3><div className="cancel-amount-list"><p><span>Subtotal</span><b>{money(breakdown.baseAmount)}</b></p>{record.discountAmount>0&&<p><span>Discount</span><b>- {money(record.discountAmount)}</b></p>}<p><span>Delivery fee</span><b>{money(record.deliveryFee)}</b></p><p className="total"><span>Original total</span><b>{money(record.originalAmount)}</b></p><p className="refund"><span>Refunded</span><b>{money(record.refundAmount)}</b></p></div></section>
...
export default function CancellationReportPage() {
  ...
  const reportPayload = { records: searchedRecords, summary: activeTab === 'refunds' ? refundSummary : summary, rangeLabel: range.label, view: activeTab, generatedBy: 'HM POS Admin' }
  ...
  const runExportXlsx = async () => {
    try {
      ...
      await addCurrentUserNotification({ category: 'exports', title: 'XLSX downloaded', message: `${activeTab === 'refunds' ? 'Refund' : 'Cancellation'} report was exported.` })
    } catch (cause) {
