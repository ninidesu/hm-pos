...
If the canonical schema is already installed, apply [`supabase/migrations/20260908090000_direct_portal_user_management.sql`](supabase/migrations/20260908090000_direct_portal_user_management.sql) once, [`supabase/migrations/20260909100000_allow_multiple_portal_users.sql`](supabase/migrations/20260909100000_allow_multiple_portal_users.sql) once, [`supabase/migrations/20260909120000_repair_portal_account_identity.sql`](supabase/migrations/20260909120000_repair_portal_account_identity.sql) once, and [`supabase/migrations/20260909130000_repair_direct_portal_auth.sql`](supabase/migrations/20260909130000_repair_direct_portal_auth.sql) once. If the earlier portal migrations were already applied, run the two latest repair migrations; the auth repair restores existing direct accounts by adding their password-login identity and complete Auth values, while the account-identity repair aligns cashier checkout with accounts stored in `public.users`. Users & Access creates confirmed local accounts directly with a username and password; it does not send invitations, emails, or OTPs and does not require an Edge Function.
